...
# # Generate canny edge image from given image"
# image = load_image(
...
# Generate a grid of images with same canny image but multiple prompts"
from PIL import Image
import os
import numpy as np
import cv2

def canny_from_original(image):
...
canny2buildings = '/export/home/cuda00022/srikanth/datasets/canny2buildings1a/test/classA'
# canny2buildings = '/export/home/cuda00022/srikanth/datasets/canny2buildings1a/test/classB'
test_images = []
canny_image = []
for i in range(1):
    path = os.path.join(canny2buildings, str(i+1)+'.jpg')
    # path = "https://hf.co/datasets/huggingface/documentation-images/resolve/main/diffusers/input_image_vermeer.png"
    test_images.append(path)
    canny = load_image(path)
    canny = Image.fromarray((255 - np.array(canny)))
    canny = canny.resize((512, 512))
    # canny = canny_from_original(path)
    canny_image.append(canny)
...
prompt = ["a high-quality, detailed, and professional image"] * len(test_images)
...
all_images = canny_image + output.images

# image_grid(output.images, 2, 2)
# grid = image_grid(all_images, 4, len(prompt)//2)
grid = image_grid(all_images, 2, len(prompt))
grid.show()
exit(0)
